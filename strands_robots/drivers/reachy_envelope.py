...
Kept in one module because two drivers need the same answer:
:class:`~strands_robots.drivers.reachy.ReachyDriver` (the ``mode="real"`` seam
driver) and :class:`~strands_robots.device_connect.reachy_mini_driver.ReachyMiniDriver`
(the Device Connect one). A second copy would let the two disagree about the same
robot, which is exactly the failure the Device Connect driver's ``look`` RPC and
this one must not have between them. It sits beside them rather than in the
``reachy_*`` agent-tool package because a driver must not need the agent surface
to bound a joint - the tool package is built on the drivers, not under them.
