...
| `download_assets` reports `failed: fetched tree has no <model_xml>` | The fetch worked - the tree is on disk - but it does not hold the file the registry entry declares as this robot's model, so `resolve_model_path` cannot find it. The clone routes fetch their source repository at HEAD, so an upstream rename retires the declared name while the directory around it survives (`so101` at Menagerie HEAD ships `so101.xml`, not the declared `so101_new_calib.xml`) | The verdict lists the model files that did arrive. Install `robot_descriptions` (`uv pip install strands-robots[sim-mujoco]`), which pins a commit of the source repo rather than following HEAD, or point the entry at a file that arrived - `add_robot` also reaches such a robot through its `scene_xml`, which is why the fetched files are left in place |
| `download_assets(action="download")` reports `error` with `No matching robots found.` | The selection matched nothing, so the call fetched nothing - a `robots=` name the registry does not list, or a `category=` no entry carries. It used to report `status: success` with `Downloaded: 0, Skipped: 0, Failed: 0` under `Method: ?` / `Assets: ?`, so a typo read as a completed download | The verdict names the unknown entries of `robots=`. `download_assets(action="list")` prints every robot it can fetch with its category; an unmatched `category=` has no name to report, which is why the count is what refuses it |
...
| `move_to`/`rotate_wrist`: `... did not reach ... The servo was stopped: the robot is in contact: 'ground' <-> 'so100/Fixed_Jaw/geom_18' (d=-0.0002 m)` (or `commanded joint(s) at a limit: '...' at its upper limit`) | The servo could not get to the value it was commanded (for `move_to`, IK had already solved the point): something on the robot is pushing against another geom, or a commanded joint sits on a bound of its range. The refusal names the pair (nearest first, at most three, total in `json.obstruction.contacts_total`) and the joint, read at the final tick. An unnamed geom is labelled `<body>/geom_<id>`, exactly as `get_contacts` labels it | Clear what the robot touches, or command a value that avoids it - a position for `move_to`, an angle for `rotate_wrist` - or one the named joint can reach inside its range. If the text says `no contact involved the robot and no commanded joint was at a limit`, nothing visible blocks it: raise `max_steps` or loosen `tol` - the arm may not have arrived yet, and a longer run is what reveals a contact that is still ahead of it. `get_contacts` shows the full list |
