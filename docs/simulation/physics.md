...
| `inverse_dynamics` | - compensation torques to hold the current `qpos`/`qvel`, one entry per DOF: a single-DOF joint under its name, a multi-DOF joint's components as `name[k]`, an unnamed joint's as `dof[i]`, named in order by `dof_joint_names` |
