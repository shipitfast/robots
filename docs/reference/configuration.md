...
| `STRANDS_ROBOTS_MICRODUCK_FRAME_ROOT` | Directory a Microduck `camera` frame is written into; `save_path` names a file inside it and a value that leaves it (absolute, or through `..`) is refused | `~/.strands_robots/microduck/frames/` |
...
| `STRANDS_GR00T_REPO_URL_ALLOW` | Comma-separated extra clone URLs added to the built-in allowlist (the canonical repo, with and without `.git`). Each entry is exact-matched, never a wildcard, so a look-alike repo cannot slip past. See [security docs](../security/policy-code.md) | built-in only |
