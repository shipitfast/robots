...
graph TB
    ...
    subgraph factory_layer[Robot factory  -  strands_robots/robot.py]
        ROBOT["Robot()"]
        REGISTRY["registry/robots.json<br/>{{n:robots}} robots, {{n:categories}} categories"]
        ROBOT --> REGISTRY
    end
...
| `strands_robots/registry/` | {{n:robots}} robots, {{n:aliases}} aliases, {{n:categories}} categories. `robots.json` is source of truth. | `list_robots()`, `resolve_name()`, `get_robot()` |
