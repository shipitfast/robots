...
class TestStartServiceEndToEnd:
    ...
    @patch.object(gi.subprocess, "run")
    ...
    @patch.object(gi, "_is_service_running", return_value=False)
    @patch.object(gi.subprocess, "run")
...
def _patch_subprocess_run(side_effect=None):
    ...
    return patch.object(gi.subprocess, "run", side_effect=side_effect)
...
class TestBuildImage:
    ...
    def test_force_rebuilds_even_when_image_exists(self, tmp_path):
        ...
        with (
            patch.object(gi, "_image_exists", return_value=True),
            patch.object(gi, "_isaac_gr00t_dir", return_value=src),
...
class TestDownloadCheckpoint:
    ...
    def test_subfolder_probe_ignores_sibling_subcheckpoints(self, tmp_path):
        ...
        fake_hub = MagicMock()
        with patch.object(gi, "require_optional", return_value=fake_hub):
            result = _download_checkpoint(
...
class TestLifecycle:
    ...
    def test_full_chains_all_steps_in_order(self, tmp_path):
        ...
        with patch.object(gi, "_build_image", side_effect=fake_build):
            with patch.object(gi, "_download_checkpoint", side_effect=fake_download):
                with patch.object(
                    gi,
                    "_start_container",
...
class TestServiceDiscovery:
    ...
    def test_status_running_maps_port_to_protocol(self):
        ...
        with patch.object(gi, "_is_service_running", return_value=True):
    ...
    def test_status_not_running_is_structured_error(self):
        ...
        with patch.object(gi, "_is_service_running", return_value=False):
    ...
    def test_stop_kills_process_inside_running_container(self):
        ...
        with (
            patch.object(gi, "_find_gr00t_containers", return_value=containers),
            patch.object(gi.subprocess, "run", side_effect=fake_run),
            patch.object(gi.time, "sleep"),
        ):
...
class TestStartRestartDispatch:
    ...
    def test_restart_requires_checkpoint_path(self):
        ...
        with patch.object(gi, "_stop_service") as stop:
    ...
    def test_restart_stops_then_starts_in_order(self):
        ...
        with (
            ...
            patch.object(gi.time, "sleep", side_effect=lambda s: calls.append("sleep")),
        ):
